...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 13 00:54:00 2018
...
def build_generator(latent_size):
    ...
    cnn.add(Dense(128 * 3 * 3, activation='relu',kernel_initializer='glorot_normal', bias_initializer='Zeros'))
    cnn.add(Reshape((128, 3, 3)))
...
#    cnn.add(LeakyReLU(alpha=0.2))    
    ...
    cnn.add(Dropout(0.2))
    cnn.add(Conv2D(64,kernel_size=2,strides=1, padding='valid',
                          activation='relu', init='glorot_normal'))    
    cnn.add(Dropout(0.2))
...
def build_discriminator():
    ...
    cnn.add(GaussianNoise(0.05, input_shape=(1, 5, 5)))  # Add this layer to prevent D from overfitting!

    cnn.add(Conv2D(16, kernel_size=3, strides=1, padding='same',
                   kernel_initializer='glorot_normal', bias_initializer='Zeros'))
    cnn.add(BatchNormalization())
    cnn.add(LeakyReLU(alpha=0.2))
    cnn.add(Dropout(0.3))
...
def build_discriminator_v2():
    image=Input(shape=(1,5,5))  
    dis=GaussianNoise(0.05, input_shape=(1, 5, 5))(image)

    dis_1=Conv2D(16, kernel_size=3, strides=1, padding='same',
                   kernel_initializer='glorot_normal', bias_initializer='Zeros')(dis)
    dis_2=BatchNormalization()(dis_1)
    dis_3=LeakyReLU(alpha=0.2)(dis_2)
    dis_4=Dropout(0.3)(dis_3)
    
    dis_5=Conv2D(32, kernel_size=2, strides=1, padding='same',
                   kernel_initializer='glorot_normal', bias_initializer='Zeros')(dis_4)
    dis_6=BatchNormalization()(dis_5)
    dis_7=LeakyReLU(alpha=0.2)(dis_6)
    dis_8=Dropout(0.3)(dis_7)

    cnn=Flatten()(dis_8)
    
    fake = Dense(1, activation='sigmoid', name='generation',
                 kernel_initializer='glorot_normal', bias_initializer='Zeros')(cnn)
    
    cnn_2=Dense(64, activation='relu', name='Dense_1',
                 kernel_initializer='glorot_normal', bias_initializer='Zeros')(cnn)
#    cnn_2=Dropout(0.3)(cnn_2)   

#    cnn_2=Dense(64, activation='relu', name='Dense_2',
#                 kernel_initializer='glorot_normal', bias_initializer='Zeros')(cnn_2)
#    cnn_2=Dropout(0.3)(cnn_2)
    
    aux = Dense(Class_num, activation='softmax', name='auxiliary',
                kernel_initializer='glorot_normal', bias_initializer='Zeros')(cnn_2)

#    acc=Model(image,[fake,aux])
...
def load_data(X,Y,percent):
    ...
    for i in range(0,len(X)):
        if i in index_test:
            ...
            x_train.append(X[i])
            y_train.append(Y[i])
    x_test=(np.array(x_test,dtype=float)).reshape(-1,1,5,5)
    y_test=np.array(y_test,dtype='uint8')
    x_train=(np.array(x_train,dtype=float)).reshape(-1,1,5,5)
    y_train=np.array(y_train,dtype='uint8')
    return x_train,y_train,x_test,y_test

def load_data_1(X,Y,percent):
    index_test=sorted(random.sample(range(0,len(X)),int(percent*len(X))))
    x_test=[]
    y_test=[]
    x_train=[]
    ...
    for i in range(0,len(X)):
        if i in index_test:
            ...
            y_train.append(Y[i])
    x_test=(np.array(x_test,dtype=float)).reshape(-1,1,5,5)
    y_test=np.array(y_test,dtype='uint8').reshape(-1,20)
    x_train=(np.array(x_train,dtype=float)).reshape(-1,1,5,5)
    y_train=np.array(y_train,dtype='uint8').reshape(-1,20)
    return x_train,y_train,x_test,y_test
...
def main(Base_dir,X0,y0,nb_epochs=100,batch_size=10):
    ...
    gen_adam=optimizers.Adam(lr=(adam_lr), beta_1=adam_beta_1)    
    ...
    generator.compile(optimizer=gen_adam,loss='binary_crossentropy')
    generator.summary()
...
#    nb_test =X_test.shape[0]
    ...
    y0=y0-6
    y0=np_utils.to_categorical(y0,Class_num)
    ...
    for epoch in range(nb_epochs):
        if epoch%10==0:
            [X_train, y_train, X_test, y_test] =load_data_1(X0,y0,0.1)
            X_train = (X_train.astype(np.float32)) 
            X_test = (X_test.astype(np.float32))
            nb_test =X_test.shape[0]
