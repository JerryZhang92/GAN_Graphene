...
def build_generator(latent_size):
   ...
   # cnn.add(Reshape((100, 1, 1)))
    ...
    cnn.add(Conv2D(128,kernel_size=1,strides=1, padding='same',
                           activation='relu', init='glorot_normal'))   
    cnn.add(BatchNormalization())
    cnn.add(Dropout(0.2))
    cnn.add(UpSampling2D(size=(2, 2)))

    cnn.add(GaussianNoise(0.08, input_shape=(256, 2, 2)))
...
#    cnn.add(LeakyReLU(alpha=0.2))    
    ...
    cnn.add(GaussianNoise(0.08, input_shape=(128, 4, 4)))
...
# =============================================================================
    ...
    h=Concatenate(axis=-1)([latent,cls])
    h=Dense(100, input_dim=latent_size, activation='relu',
                  kernel_initializer='glorot_normal', bias_initializer='Zeros')(h)
...
def main(Base_dir,X0,y0,nb_epochs=100,batch_size=10):
    ...
    gen_adam=optimizers.Adam(lr=(adam_lr*2), beta_1=adam_beta_1)    
    # build the discriminator
    discriminator = build_discriminator()
#    discriminator.compile(optimizer=dis_adam,loss=['binary_crossentropy', 'sparse_categorical_crossentropy'])
    discriminator.compile(optimizer=dis_adam,loss=['binary_crossentropy', 'categorical_crossentropy'])
    discriminator.summary()
    # build the generator
    generator = build_generator(latent_size)
    generator.compile(optimizer=gen_adam,loss='mean_squared_error')
    generator.summary()
...
#    Adversarial_Net.compile(optimizer=gen_adam,loss=['binary_crossentropy', 'sparse_categorical_crossentropy'])
...
#                    aux_y2 = sampled_labels_index    #### one hot sampled_labels
...
#                [noise, sampled_labels_index.reshape((-1, 1))], [trick, sampled_labels_index]))  ### one-hot    sampled_labels
            epoch_gen_loss.append(Adversarial_Net.train_on_batch(
                    [noise, sampled_labels_index.reshape((-1, 1))], [trick, sampled_labels]))  ### one-hot    sampled_labels
...
###########################################################################################################  
#        aux_y = np.concatenate((y_test.reshape(-1, ), sampled_labels_index), axis=0)
#        aux_y = np.concatenate((y_test, sampled_labels_index), axis=0)      ## one-hot sampled_labels
...
###########################################################################################################  
        generator_test_loss = Adversarial_Net.evaluate(
            ...
            [trick, sampled_labels], verbose=False)   ### one-hot sampled_labels        
#        generator_test_loss = Adversarial_Net.evaluate(
#            [noise, sampled_labels_index.reshape((-1, 1))],
#            [trick, sampled_labels_index], verbose=False)   ### one-hot sampled_labels
###########################################################################################################  
