...
def structure_smoothing(temp,size):
    for i in range(size):
        for j in range(size):
            if temp[0,i,j,0]==0:
                temp[0,i,j,0]=np.random.normal(0.4,0.2)
            else:
                temp[0,i,j,0]=np.random.normal(0.8,0.2)
    return temp
...
def load_data(local_dir,Type):
    ...
    for line in fp_text_x:
        tmp_count+=1
        if tmp_count%1000==0:
            print(tmp_count,' number of data have been loaded\n')
        col=line.split("\t")
        if '' in col:
            col.remove('')
        ...
        if Type=='6by6'or Type=='6by6_test':
            ...
            size=6
        temp=structure_smoothing(temp,size)
        if data_x=='':
            data_x=temp
        else:
...
#             elif 15<=i<20:
#                i=np.array(2,dtype=int)
#             elif 20<=i<=25:
#                i=np.array(3,dtype=int)
