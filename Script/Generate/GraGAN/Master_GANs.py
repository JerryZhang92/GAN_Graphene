...
###########################################################################################################
    ...
    if Type=='5by5':
        Input_X,Input_Y=Data_processing_GANs.load_data(Base_dir,'5by5')        
...
####0#############################################################G##########################################
    ...
    if Type=='5by5':
        [Gen,Dis,train,test]=ACGAN_5by5.main(Base_dir,Input_X,Input_Y,nb_epochs=Epc,batch_size=256)        
#    Epc_Change=''
...
#        a2=a1*2
#        a3=a1*3
#        sampled_labels=np.concatenate((a0,a1,a2,a3),axis=0)       
##############################
        ...
        if Type=='4by4':
            ...
            for c in range(len(generated_data)):
                for i in range(4):
                    for j in range(4):
                        if generated_data[c,i,j]>0.6:
                            generated_data[c,i,j]=1
                        else:
                            generated_data[c,i,j]=0            
        elif Type=='5by5':
            generated_data=generate_data.reshape(sample_data_num,5,5,1)
            for c in range(len(generated_data)):
                for i in range(5):
                    for j in range(5):
                        if generated_data[c,i,j]>0.6:
...
###############################################                            
        ...
        #generate_labels=(sampled_labels*5+2.5)/10
###############################################        
    ...
    if  per_error_sum<13.956085 and Type=='4by4':
        Epc_Change='Best_4by4'        
        Data_processing_GANs.save(Gen,Base_dir,Epc_Change,version,Type='Generator')
        Data_processing_GANs.save(Dis,Base_dir,Epc_Change,version,Type='Discriminator')        
    elif Type=='5by5'and per_error_sum<16.552576:
        Epc_Change='Best_5by5'        
        Data_processing_GANs.save(Gen,Base_dir,Epc_Change,version,Type='Generator')
        Data_processing_GANs.save(Dis,Base_dir,Epc_Change,version,Type='Discriminator')        
...
#    plot_model(Dis,to_file=Dis_model_dir)
    ...
    return rela_error_sum,abs_error_sum,per_error_sum,Gen,Result